...
size_t AccumulableDose::FlattenVoxelIndex(const G4Vox::VoxelIndex &v) const
{
    // Compute the flat voxel index from the 3D index
    return v.Flatten(this->fNx, this->fNy);
}

G4Vox::VVoxQuantityAccumulable *QuantityDose::UserCreateAccumulable(const G4String &name) const
{
    return new AccumulableDose(name, this->GetMaxVoxIndex()); // pass weak_ptr
}

void QuantityDose::ReadAccumulable(const G4Vox::VVoxQuantityAccumulable &other)
{
