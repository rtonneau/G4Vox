...
namespace G4Vox
{
    namespace Quantities
    {
        ...
        void AccumulableDose::ScoreImpl(const G4Step *aStep)
        {
        ...
        VVoxQuantityAccumulable *Dose::UserCreateAccumulable(const G4String &name) const
        {
            return new AccumulableDose(name, this->GetMaxVoxIndex()); // pass weak_ptr
        }

        void Dose::ReadAccumulable(const VVoxQuantityAccumulable &other)
        {
