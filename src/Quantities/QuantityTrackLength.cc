...
size_t AccumulableTrackLength::FlattenVoxelIndex(const G4Vox::VoxelIndex &v) const
{
    // Compute the flat voxel index from the 3D index
    return v.Flatten(this->fNx, this->fNy);
}

G4Vox::VVoxQuantityAccumulable *QuantityTrackLength::UserCreateAccumulable(const G4String &name) const
{
    return new AccumulableTrackLength(name, this->GetMaxVoxIndex()); // pass weak_ptr
}

void QuantityTrackLength::ReadAccumulable(const G4Vox::VVoxQuantityAccumulable &other)
{
    const auto &o = static_cast<const AccumulableTrackLength &>(other);
