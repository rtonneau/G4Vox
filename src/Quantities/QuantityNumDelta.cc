#include "G4Vox/Quantities/QuantityNumDelta.hh"
...
size_t AccumulableNumDelta::FlattenVoxelIndex(const G4Vox::VoxelIndex &v) const
{
    // Compute the flat voxel index from the 3D index
    return v.Flatten(this->fNx, this->fNy);
}

G4Vox::VVoxQuantityAccumulable *QuantityNumDelta::UserCreateAccumulable(const G4String &name) const
{
    return new AccumulableNumDelta(name, this->GetMaxVoxIndex()); // pass weak_ptr
}

void QuantityNumDelta::ReadAccumulable(const G4Vox::VVoxQuantityAccumulable &other)
{
