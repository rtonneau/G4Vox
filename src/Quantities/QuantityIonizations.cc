#include "G4Vox/Quantities/QuantityIonizations.hh"
...
size_t AccumulableIonizations::FlattenVoxelIndex(const G4Vox::VoxelIndex &v) const
{
    // Compute the flat voxel index from the 3D index
    return v.Flatten(this->fNx, this->fNy);
}

G4Vox::VVoxQuantityAccumulable *QuantityIonizations::UserCreateAccumulable(const G4String &name) const
{
    return new AccumulableIonizations(name, this->GetMaxVoxIndex()); // pass weak_ptr
}

void QuantityIonizations::ReadAccumulable(const G4Vox::VVoxQuantityAccumulable &other)
{
