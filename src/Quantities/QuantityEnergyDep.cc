...
size_t AccumulableEnergyDep::FlattenVoxelIndex(const G4Vox::VoxelIndex &v) const
{
    // Compute the flat voxel index from the 3D index
    return v.Flatten(this->fNx, this->fNy);
}

G4Vox::VVoxQuantityAccumulable *QuantityEnergyDep::UserCreateAccumulable(const G4String &name) const
{
    return new AccumulableEnergyDep(name, this->GetMaxVoxIndex()); // pass weak_ptr
}

void QuantityEnergyDep::ReadAccumulable(const G4Vox::VVoxQuantityAccumulable &other)
{
