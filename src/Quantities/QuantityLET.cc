...
{
    G4Vox::VVoxQuantityAccumulable::Initialize();
    this->fTotLength = G4Vox::array_type(0.0, this->TotalVoxels());
}
...
size_t AccumulableLET::FlattenVoxelIndex(const G4Vox::VoxelIndex &v) const
{
    // Compute the flat voxel index from the 3D index
    return v.Flatten(this->fNx, this->fNy);
}

G4Vox::VVoxQuantityAccumulable *QuantityLET::UserCreateAccumulable(const G4String &name) const
{
    return new AccumulableLET(name, this->GetMaxVoxIndex()); // pass weak_ptr
}

void QuantityLET::ReadAccumulable(const G4Vox::VVoxQuantityAccumulable &other)
{
    const auto &o = static_cast<const AccumulableLET &>(other);
